	...
	{{ Form::open(['class' => 'form-signin', 'autocomplete' => 'off']) }}
		<div style="padding:15px;background-color:#fff;border-radius:6px;">
			...
			<div class="{{{ $errors->has('name') ? 'error' : '' }}}">
				<input type="text" id="name" name="name" placeholder="Nombre.." required="true" class="form-control" value="{{{ Request::old('name', $auth->name) }}}" />
				{{{ $errors->first('name') }}}
			</div>
			
			<div class="{{{ $errors->has('email') ? 'error' : '' }}}">
				<input type="email" id="email" name="email" placeholder="Correo Electrónico.." required="true" class="form-control inputs" value="{{{ Request::old('email', $auth->email) }}}" />
				{{{ $errors->first('email') }}}
			</div>
